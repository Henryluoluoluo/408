...
22.
1)
算法思想:采用双指针算法,先移动一个指针，当其移动k次后，开始同步移动另一个指针，直到先移动的指针到达null
2)
步骤描述:
1. 将两个指针p和q同时指向第一个元素，初始化count = 0
2.重复以下步骤，直至p指向空
  2.1 若 count < k: count ++
      否则 q = q -> next;
  2.2 p = p -> next;
3.若count < k:说明链表中元素不到k个，返回false
4.输出q内元素 返回true
...
```
23.对带头结点的两个单链表求公共后缀的起始节点
1) 算法思路:
    1.先求两个单链表的长度l1,l2
    2.让长度较短的单链表先移动|l1 - l2|个位置
    3.重复以下操作直至到末尾
        3.1若所在结点的值相等，退出循环
        3.2 两个指针指向下一个结点
    4.返回任意一个结点
...
```
24.
1)采用以空间换时间的方法，初始化一个st[N]数组，标记出现过的数
    1.重复以下操作直至所有访问完所有单链表中结点
        1.1若当前指向结点的数存在过，删除该结点
        1.2将指针后移
...
25. 将单链表重排序
1) 算法思想:
    1.将后半段链表提取出来，将后半段逆置，然后再插入

2) 
    void change(NODE *h) {
        NODE *p,*q;
        //p每次向后移动一个单位，q每次向后移动两个单位
        while(q -> next != null) {
            p = p -> next;
            q = q -> next;
            if(q -> next != null)
